...
class Monovariable {
  ...
  */
  public static double[][] useQuadraticFormula(double aNum, double bNum, double cNum) {
    ...
    double[] discriminant1 = calcComp(new double[]{4,0}, '*', calcComp(aSolution, '*', cSolution)); // 4ac

    // Full discriminant
    double[] discriminant = calcComp(discriminant0, '-', discriminant1);

    // Solution with plus sign
    double[] xPlus = calcComp(
      calcComp(
        new double[] {-bNum, 0.0}, 
        '+', 
        calcComp(
  ...
  public static double[][] useCubicFormula(double aNum, double bNum, double cNum, double dNum) // CUBIC FORMULA
  {
    ...
    double[] delta0 = calcComp(calcComp(bSolution, '^', new double[] {2.0, 0}), '-', calcComp(calcComp(aSolution, '*', cSolution), '*', new double[] {3.0, 0}));
      
      //calcComp(bSolution, '^', new double[] {2, 0}), '-', calcComp(new double[] {3,0}, '*', calcComp(aSolution, '*', cSolution)));     
    ...
    /*double[] delta1 = calcComp(calcComp(calcComp([2*bNum, 0], '^', new double[] {3.0, 0}), '-', new double[] {9*aNum*bNum*cNum,0}), '+', calcComp(calcComp(new double[] {aNum*27, 0}, '^', new double[] {2.0, 0}), '*', dSolution)*/
    ...
    double[] C6 = calcComp(C5, '2', new double[] {});
    double[] C7 = calcComp(delta1, '+', C6);
    double[] C8 = calcComp(C7, '/', new double[] {2,0});
    double[] C = calcComp(C8, '3', new double[] {});

    
    /*double[] x = calcComp(new double[]{-1,0}, '/', calcComp(new double[]{3,0}, '*', aSolution), '*', (calcComp(calcComp(bSolution, '+', C), '+', calcComp(delta0, '/', C))));*/
   ...
   double[] x0 = calcComp(calcComp(new double[]{-1,0}, '/', (calcComp(new double[]{3,0},'*', aSolution))), '*', calcComp( calcComp(bSolution, '+', C), '+', calcComp(delta0, '/', C)));
   double[] x1 = calcComp(calcComp(new double[]{-1,0}, '/', (calcComp(new double[]{3,0},'*', aSolution))), '*', calcComp( calcComp(bSolution, '+', calcComp(xi, '*', C)), '+', calcComp(delta0, '/', calcComp(xi, '*', C))));
   double[] x2 = calcComp(calcComp(new double[]{-1,0}, '/', (calcComp(new double[]{3,0},'*', aSolution))), '*', calcComp( calcComp(bSolution, '+', calcComp(xi_sqr, '*', C)), '+', calcComp(delta0, '/', calcComp(xi_sqr, '*', C))));
  ...
  public static double[] useCubicFormulaLambda(double[] aSolution, double[] bSolution, double[] cSolution, double[] dSolution) // CUBIC FORMULA, BUT ONLY RETURNS ONE ANSWER
  {
    ...
    double[] result2 = calcComp(C, '*', calcComp(-1, '+', calcComp(calcComp(-3, '2', new double[] {}), '/', 2)));

    //double result3 = bigC * ((-1 - Math.sqrt(-3))/2);
    double[] result3 = calcComp(C, '*', calcComp (-1, '-', calcComp(calcComp(-3, '2', new double[] {}), '/', 2)));
    */
  ...
  public static double[][] useQuarticFormula(double aNum, double bNum, double cNum, double dNum, double eNum) { // QUARTIC FORMULA
      ...
      double[] p = calcComp(bSolution, '-', calcComp(new double[] {3,0}, '*', calcComp(calcComp(aSolution, '*', aSolution), '/', new double[] {8,0})));  
      
       
      double[] abOver2 = calcComp(calcComp(aSolution, '*', bSolution), '/', new double[] {2,0}); // (Singh, 2020), Jared stole this.
      double[] aCubedOver8 = calcComp(
      ...
      double[] aSquaredBOver16 = calcComp(calcComp(bSolution, '*',calcComp(aSolution, '^', new double[] {2,0})), '/', new double[] {16,0});
      double[] threeAFourthOver256 = calcComp(calcComp(calcComp(aSolution, '^', new double[] {4,0}), '*', new double[] {3,0}), '/', new double[] {256,0});
      ...
      double[] lambda = useCubicFormulaLambda(new double[] {1,0}, calcComp(new double[] {2,0},'*',p),calcComp(calcComp(p, '*', p), '-', calcComp(new double[] {4,0}, '*', r)), calcComp(new double[] {-1, 0}, '*', calcComp(q, '*', q)));


      //(+,+) 
      //The last three lines from this and sqrtLambda are just given different itterations
      double[] sqrtLambda = calcComp(lambda, '2', new double[] {});
      ...
      double[] innerSqrt = calcComp(lambdaMinusTwoTimesInnerParen, '2', new double[] {});
      ...
      //(-,-)
      double[] negativeSrtLambda  = calcComp(new double[] {-1,0}, '*', calcComp(lambda, '2', new double[] {}));
...
/**
   ...
   * ComplexService.calculateComplex()
  */
