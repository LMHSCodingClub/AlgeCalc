...
class ComplexService {
	 ...
	 * DM: (cosx + i*sin x)^n = cos (nx) + i*sin(nx) modification: z = r(cos x + i sin x)
	 * r^(1/n) ( cos((x+2pi*k)/(n)) + i*sin((x+2pi*k)/n))
	 ...
	 */
	public static double[] findSquareRoot(double[] values) {
		if (values[0] >= 0 && values[1] >= 0) { // If the square root will
			return new double[] { Math.sqrt(values[0]), Math.sqrt(values[1]) };
		}
		...
		if (values[1] == 0) {
			toReturn[0] = 0.0;
			toReturn[1] = Math.pow(Math.abs(values[0]), 1.0 / 2.0);
		} else {
			double[] polar = polarize(values);
			// You add a 2pi*k for other roots
			toReturn[0] = Math.pow(polar[0], 1.0 / 2.0) * Math.cos((polar[1] / (2)));
			// You add a 2pi*k for other roots
			toReturn[1] = Math.pow(polar[0], 1.0 / 2.0) * Math.sin((polar[1] / (2)));
		}
